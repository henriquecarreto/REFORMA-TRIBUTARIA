...
export const ShowcaseGallery: React.FC = () => {
  ...
  // Quadruplicado para garantir fluxo contínuo e infinito no marquee em telas de qualquer tamanho
  ...
  return (
    <section className="py-10 sm:py-16 bg-[#FAF6F0] border-b border-amber-200/60 overflow-hidden">
      ...
      {/* CARROSSEL DE FOTOS EM MOVIMENTO AUTOMÁTICO (MOBILE E DESKTOP) */}
      <div className="w-full overflow-hidden relative py-2">
        <div className="animate-marquee flex items-center gap-3.5 sm:gap-6 px-2">
          {infiniteLifestyle.map((photo, idx) => (
            <div
              key={`${photo.id}-${idx}`}
              onClick={() => setSelectedImage(photo.src)}
              className="shrink-0 w-[260px] min-[380px]:w-[290px] sm:w-[350px] md:w-[440px] lg:w-[480px] cursor-pointer group/item transition-transform duration-300 hover:scale-[1.015]"
            >
              <div className="relative rounded-2xl overflow-hidden bg-white border border-amber-200/80 shadow-md group-hover/item:shadow-xl transition-all">
                <img
                  src={photo.src}
                  alt={photo.alt}
                  className="w-full h-auto max-h-[240px] sm:max-h-[300px] md:max-h-[380px] lg:max-h-[420px] object-cover rounded-xl"
                  loading="lazy"
                />

                <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover/item:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-2 text-white font-bold text-xs sm:text-base p-4 text-center">
                  <div className="p-2.5 sm:p-3 rounded-full bg-[#00A859] text-white shadow-lg">
